...
import { ArticleCategory, ArticleRow, readMinutesOf } from "@/lib/supabase/articles";
...
export default function ArticlesPageContent({ articles }: { articles: ArticleRow[] }) {
  const t = useTranslations("Articles");
  const tNav = useTranslations("Nav");
  const tEmpty = useTranslations("EmptyState");
  const locale = useLocale();
  const [cat, setCat] = useState<ArticleCategory | "الكل">("الكل");

  const CATEGORIES: { key: ArticleCategory | "الكل"; label: string }[] = [
      ...
      {filtered.length === 0 ? (
        ...
        <div key={cat} className="animate-fade-up mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((a) => {
            ...
            const excerpt = locale === "en" && a.excerpt_en ? a.excerpt_en : a.excerpt;
            return (
              <Link key={a.slug} href={`/articles/${a.slug}`} className="card-interactive flex flex-col gap-4 p-6">
                <div className="relative aspect-[16/10] overflow-hidden rounded-2xl bg-bg">
                  <Image
                    src={a.image || "/icon.png"}
                    alt={title}
                    fill
                    sizes="(min-width: 1024px) 380px, 90vw"
                    className={a.image ? "object-cover" : "object-contain p-10"}
                  />
                </div>
                <span className="badge w-fit">{t(CATEGORY_KEYS[a.category])}</span>
                <h2 className="text-lg font-extrabold leading-snug text-ink">{title}</h2>
                <p className="text-sm leading-relaxed text-ink-soft">{excerpt}</p>
                <div className="mt-auto flex items-center justify-between border-t border-line pt-4">
                  <span className="text-xs text-ink-soft">
                    {a.created_at.slice(0, 10)} · {readMinutesOf(a.content)} {t("readMinutes")}
                  </span>
